...
2. **MCP Server** — a FastMCP HTTP/SSE server that talks IMAP/SMTP over `127.0.0.1`.

The bridge only accepts local connections, so co-locating in the same network namespace is required. The MCP server exposes SSE on `0.0.0.0:8000`; Traefik on the `proxy` network routes external clients to it.
...
Provider-agnostic mail operations built on IMAP.

```text
EmailClient (abstract)
    └─ ImapClient
           └─ ProtonMailClient
```
...
`ImapClient` implements the contract using `imapclient` + STARTTLS. It exposes small provider hooks that subclasses override:
...
| `draft_email` | `mailbox`, `to`, `subject`, `body`, `cc`, `bcc`, `reply_to_message_id` | Append a draft to Drafts. |
...
- IMAP/SMTP connections are localhost-only and use STARTTLS with the bridge’s self-signed cert.
- Optional `MCP_API_KEY` env var can require an `Authorization` header on the SSE endpoint if set.
